...
const tsconfigCache = new Map<string, { paths: ReturnType<typeof createPathsMatcher>;
 ...
 */
export const createDynamicPathsMatcher = () => (specifier: string, fromFile?: string): string[] => {
	if (!fromFile) {
	...
	logCjs(3, 'createDynamicPathsMatcher', {
		specifier,
		fromFile,
	});

	// Find the directory to start searching from
	const searchDir = path.dirname(fromFile);
	...
	if (tsconfigCache.has(searchDir)) {
		...
		if (cached?.paths) {
			...
			logCjs(3, 'createDynamicPathsMatcher cached result', {
				searchDir,
				specifier,
				result,
			});
		...
		logCjs(3, 'createDynamicPathsMatcher cached null', {
			searchDir,
			specifier,
		});
		return [];
	...
	try {
		...
		logCjs(2, 'createDynamicPathsMatcher error parsing tsconfig', {
			tsconfigPath,
			error,
		});
		tsconfigCache.set(searchDir, null);
