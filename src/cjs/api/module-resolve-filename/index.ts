	...
	if (
		...
		if (parent?.filename && dynamicPathsMatcher) {
			try {
				...
				log(2, 'resolveTsPaths dynamic matcher error', { error, request, parentFilename: parent.filename })
			}
		}
		...
		if (possiblePaths.length === 0 && tsconfigPathsMatcher) {
			try {
				...
				log(3, 'resolveTsPaths static fallback', { request, possiblePaths })
			} catch (error) {
				log(2, 'resolveTsPaths static matcher error', { error, request })
			}
		}

		log(3, 'resolveTsPaths final paths', { request, possiblePaths })
