...
import { ptyShell, isWindows, quoteForShell } from '../utils/pty-shell/index'
			...
			describe('Ctrl + C', ({ test }) => {
				...
				test('Exit code', async ({ onTestFail }) => {
					const p = ptyShell(async (shell) => {
						...
						shell.type(`${quoteForShell(node.path)} ${tsxPath} ${fixture.getPath('keep-alive.js')}`)
				...
				test('Catchable', async ({ onTestFail }) => {
					const p = ptyShell(async (shell) => {
						await shell.waitForPrompt()
						shell.type(`${quoteForShell(node.path)} ${tsxPath} ${fixture.getPath('catch-signals.js')}`)
				...
				test('Infinite loop', async ({ onTestFail }) => {
					const p = ptyShell(async (shell) => {
						await shell.waitForPrompt()
						// Windows doesn't support shebangs
						shell.type(`${quoteForShell(node.path)} ${tsxPath} ${fixture.getPath('infinite-loop.js')}`)
